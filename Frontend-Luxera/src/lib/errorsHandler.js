export const handleAuthErrors = (error) => {
    if (error.response && error.response.data) {
      ...
      return { general: { message: data.message || "An unknown error occurred. Please try again later." } };
    }
  
    // Handle other cases where error.response.data is not present
    console.error("Error:", error);
    return { general: { message: "An unexpected error occurred, please try again later." } };
  };
